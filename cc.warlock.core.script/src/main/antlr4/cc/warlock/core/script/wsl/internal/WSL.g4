...
expr returns [WSLAbstractCommand command]
	: IF BLANK* cond=conditionalExpression BLANK* THEN BLANK* c=expr
		{
			command = new WSLCondition(lineNum, script, cond, c);
		}
	| ACTION BLANK* { actionDepth++; }
		( REMOVE BLANK* { actionDepth--; } args=string_list
			{
...
quoted_string_value returns [IWSLValue value]
	: v=variable { value = v; }
	| BACKSLASH t=QUOTE { value = new WSLString($t.text); }
	| str=text_string { value = new WSLString(str); }
	;
...
vstring_value returns [IWSLValue value]
	...
	| (PERCENT t=PERCENT
		| DOLLAR t=DOLLAR
		| t=QUOTE | BACKSLASH t=LPAREN
		| BACKSLASH t=RPAREN | t=BLANK)
